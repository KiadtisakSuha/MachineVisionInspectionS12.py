...
class Getpart():
    ...
    def Get(self):
        ...
        return [self.PartNumber, self.BatchNumber, self.PartName, self.CustomerPartNumber, self.MachineName,
                self.MoldId, self.Sever, self.Packing,self.Status]
...
class Image_Processing:
    ...
    def rbg2hsv(image):
        ...
        s_mean = np.mean(s)
        v_mean = np.mean(v)
        return (int(h_mean), int(s_mean), int(v_mean))
    @staticmethod
    def hsv_processing(master_color,image_color):
        color_diff = []
        for i in range(len(master_color)):
            if image_color[i] > master_color[i]:
                diff = (master_color[i] * 100) / image_color[i]
            else:
                diff = (image_color[i] * 100) / master_color[i]
            color_diff.append(diff)
        avg_diff = int(sum(color_diff) / len(master_color) * 10)
        return avg_diff
...
class Save_Data:
    ...
    @staticmethod
    def Save_Master(Left, Top, Right, Bottom, Score_Outline, Score_Area, Cam, Point, Emp_ID, Partnumber,Mode,hsv):
        ...
        try:
                ...
                for i in range(26):
                    ...
                    try:
                        if Point == "Point" + str_:
                            ...
                            item[i]["Point" + str_][0]["HSV Colour Scale"] = hsv
                        ...
                        try:
                            ...
                            item.append({'' + Point + '': [
                                {"Emp ID": Emp_ID,"Mode": Mode ,"Camera": Cam, 'Left': Left, 'Top': Top, "Right": Right, "Bottom": Bottom,
                                 'Score Outline': Score_Outline, 'Score Area': Score_Area, 'HSV Colour Scale':hsv}]})
                            with open(Partnumber + '/' + Partnumber + '.json', 'w') as json_file:
                                json.dump(item, json_file, indent=6)
                        except:
                            pass
            ...
            if Point == "Point1":
                item = [
                    {'' + Point + '': [
                        {"Emp ID": Emp_ID,"Mode": Mode, "Camera": Cam, 'Left': Left, 'Top': Top, "Right": Right, "Bottom": Bottom, 'Score Outline': Score_Outline, 'Score Area': Score_Area,'HSV Colour Scale':hsv}]}]
                with open(Partnumber + '/' + Partnumber + '.json', 'w') as json_file:
                    json.dump(item, json_file, indent=6)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.Status = self.API_json.Get()[8]
        self.Batch_API_Get = self.Batch_API
        self.PartNumber = self.Part_API + "|" + "|"
        ...
        self.view_Camera2.place(x=780, y=300)
        ...
        self.Process.place(x=395, y=220, height=60, width=310)
        ...
        self.btn_Start.place(x=1000, y=10, width=120)
        ...
        """if Mode == 3:
            try:
                ...
                self.server = InfiniteTimer(0.1, self.server_program)
                self.server.start()
            except:
            ...
            try:
                ...
                self.Ready = False
                self.client = InfiniteTimer(0.1, self.client_program)
                self.client.start()
            except:
                messagebox.showwarning("TCP IP", "Address or Port doesn't match")"""
        ...
        self.btn_add.place(x=1350, y=10)
        ...
        self.btn_Close.place(x=1500, y=10, width=100)
    ...
    def AddMaster(self):
        ...
        self.buttonLogin.configure(font=("Arial", 20))
        self.buttonLogin.configure(justify="center", foreground="green")
        self.buttonLogin.place(x=35, y=50, width=150, height=50)
    ...
    def Search(self):
        if self.Loginform():
            ...
            Lable_Mode = tk.Label(self.SaveMaster, text="Mode :", bg='#D4D4D4')
            Lable_Mode.configure(font=("Arial", 20))
            Lable_Mode.configure(fg='Green')
            Lable_Mode.place(x=10, y=110)

            Mode_Data = tk.StringVar()
            Mode_Point = ttk.Combobox(self.SaveMaster, width=8, height=80, textvariable=Mode_Data)
            Mode_Point.configure(font=("Arial", 20))
            Mode_Point.configure(justify="center", foreground="green")
            Mode_Point['values'] = ('Color', 'Shape')
            Mode_Point.current(0)
            Mode_Point.place(x=120, y=110)
            ...
            Lable_Score_Outline.place(x=10, y=160)
            ...
            Score_Show_Outline.place(x=120, y=160, width=150)
            ...
            Lable_Score_Area.place(x=10, y=210)
            ...
            Score_Show_Area.place(x=120, y=210, width=150)
                        ...
                        def click_and_crop(event, x, y, flags, param):
                            ...
                            if event == cv.EVENT_LBUTTONDOWN:
                                ...
                                if len(refPt) == 2:
                                    ...
                                    image_crop = cv.cvtColor(roi, cv.COLOR_BGR2RGB)
                                    Left = refPt[0][0]
                                    Top = refPt[0][1]
                                    Right = refPt[1][0]
                                    Bottom = refPt[1][1]
                                    ...
                                    hsv = Image_Processing.rbg2hsv(image_crop)
                                    cv.imwrite('' + Master + '/' + Point + '_Template.bmp', image)
                                    if Left and Top and Right and Bottom != 0:
                                        Save_Data.Save_Master(Left, Top, Right, Bottom, Score_Outline, Score_Area, Cam, Point, Emp_ID, self.Part_API,Mode,hsv)
            ...
            buttonSave.place(x=120, y=260, width=150, height=80)
            ...
            if self.count == len(self.Master):
                if self.count != 0:
                    ...
                    for k in range(self.count):
                        ...
                        self.Point_Mode.append(self.Master[k]["Point"+ str(k+1)][0]["Mode"])
                        ...
                        self.Point_Score_Area.append(self.Master[k]["Point" + str(k + 1)][0]["HSV Colour Scale"])
                        #self.Point_HSV.append(self.Master[k]["Point" + str(k + 1)][0]["Score Area"])
    ...
    def Processing(self):
        if self.data == "Snap01" and self.Part_API != "":
            Login = False
            SaveMaster = False
            try:
    ...
    def Strat(self):
        if self.count != 0 and self.Part_API != "":
            self.Close_Camera = True
            self.ProcessP.configure(text="Process")
            self.ProcessP.configure(fg='yellow')
            Save_Data.Save_Imaga_Run(self.Run_Camera_1, self.Run_Camera_2, self.Run_Camera_3)
